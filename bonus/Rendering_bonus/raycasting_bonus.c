...
int wall_hit(double x, double y, t_game *g, t_ray *ray)
{
    ...
    {
        if (ft_strchr("1DR", g->data->map[y1][x1]))
            return (0);
		else if (ray->skip_closed_door && g->data->map[y1][x1] == 'd')
			return (0);
		else
        	return (1);
    }
...
t_point ray_hor_intersect(t_game *g, t_ray *ray, t_point delta)
{
	...
	if (inter.x > 0 && inter.x < g->data->width * SIZE && inter.y > 0 && inter.y < g->data->height * SIZE)
		ray->hhit_content = g->data->map[(int)floor((inter.y - pixel) / SIZE)][(int)floor(inter.x / SIZE)];
	return (inter);
}
...
t_point ray_ver_intersect(t_game *g, t_ray *ray, t_point delta)
{
	...
	if (inter.x > 0 && inter.x < g->data->width * SIZE && inter.y > 0 && inter.y < g->data->height * SIZE)
		ray->vhit_content = g->data->map[(int)floor(inter.y / SIZE)][(int)floor((inter.x - pixel) / SIZE)];
	return (inter);
}
