...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/09/29 14:37:26 by moel-gha          #+#    #+#             */
/*   Updated: 2024/09/29 20:51:29 by moel-gha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_identifier(t_game *content)
{
	...
	while (content->data->content[i])
	{
		if (ft_strcmp(content->data->content[i], "\n") == 0
			...
			|| ft_strncmp(content->data->content[i], "D", 1) == 0
			|| ft_strncmp(content->data->content[i], "R", 1) == 0)
		{
			i++;
			continue ;
		}
